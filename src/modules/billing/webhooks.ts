 ...
 */
export const billingWebhooks = new Elysia({ prefix: '/webhooks/recurrente' }).post('/', async ({ request, set }) => {
  ...
  if (event.kind === 'payment_succeeded') {
    ...
    if (inserted && event.metadata?.kind !== 'credit_topup') {
      await db.update(subscriptions).set({ status: 'past_due' }).where(eq(subscriptions.companyId, companyId));
      ...
      console.error('[billing] payment failed, subscription past_due', { companyId, eventId: event.eventId });
    }
