...
import { deliverEmail, deliverPlatformEmail, type EmailSendPayload } from '@/lib/email';
...
export function startEmailSendWorker(): Promise<string> {
  return registerWorker<EmailSendPayload>(QUEUES.emailSend, async (payload) => {
    // Correos de plataforma (demo request): sin empresa que scopear ni fila en `notifications`.
    if (payload.companyId == null) {
      await deliverPlatformEmail(payload);
      return;
    }
