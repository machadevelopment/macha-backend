...
- **⚠️ EL LOGIN POR `macha.finance` ESTÁ ROTO: WorkOS solo acepta el dominio de Vercel** (verificado 2026-08-21, reporte de Keneth "inicio sesión y me lleva a `?auth_error=1`"). Entrando por `macha.finance`, el middleware manda a WorkOS con `redirect_uri=https://macha-finance.vercel.app/callback` (el valor de `NEXT_PUBLIC_WORKOS_REDIRECT_URI` en Vercel), así que después de autenticar el usuario **aterriza en otro dominio** — donde no está la cookie del flujo — y sale `?auth_error=1`. Probado contra la API de WorkOS: `https://macha.finance/callback` se rechaza con `redirect-uri-invalid`, exactamente igual que un dominio inventado; el único registrado es el `.vercel.app`.
  **El arreglo tiene DOS pasos y el ORDEN no es negociable:** (1) registrar `https://macha.finance/callback` en los *Redirects* del dashboard de WorkOS, y (2) recién después cambiar `NEXT_PUBLIC_WORKOS_REDIRECT_URI` en Vercel. Al revés queda peor que ahora: hoy el login falla DESPUÉS de autenticar; con la variable cambiada y la URI sin registrar, WorkOS corta antes de mostrar la pantalla de login.
  **Y el problema de fondo no es la variable, son los CUATRO dominios** que apuntan al mismo proyecto de Vercel (`macha.finance`, `macha-finance.vercel.app`, `macha-finance-macha6.vercel.app`, `macha-finance-git-main-macha6.vercel.app`). Con un `redirect_uri` fijo, el login solo funciona bien entrando por UNO; por cualquier otro el usuario termina saltando de dominio. Mientras no haya un canónico con los demás redirigiendo a él, este bug vuelve por la puerta de al lado.
